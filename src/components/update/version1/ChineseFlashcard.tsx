...
// --- Vocabulary Lists (batches) ---


// Central config for all available sets
...
export default function ChineseFlashcardApp() {
  ...
  // If list is empty, show a simple fallback
  if (vocabularyList.length === 0) {
    return (
      <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center py-8 px-4 font-sans text-slate-800">
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col items-center py-8 px-4 font-sans text-slate-800">
      ...
      <header className="mb-8 text-center">
        <div className="flex items-center justify-center gap-2 mb-2 text-red-600">
          <BookOpen className="w-8 h-8" />
          <h1 className="text-3xl font-bold tracking-tight">Daniel Ding  学好中文，好幸运</h1>
        </div>
      ...
      {/* Main Card Container */}
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-200 transition-all duration-300 hover:shadow-2xl">
        {/* Progress Bar */}
        ...
        <div className="flex flex-col gap-2 px-6 py-4 bg-slate-50 border-b border-slate-100">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <input
              ...
              className="w-20 rounded-md border border-slate-300 px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-red-400"
        ...
        <div className="bg-slate-50 p-6 flex justify-between items-center border-t border-slate-200">
          <button
            ...
            className={`p-4 rounded-full transition-all shadow-sm flex items-center justify-center ${
              safeIndex === 0
          ...
          <button
            onClick={handleReset}
            className="flex flex-col items-center justify-center gap-1 text-slate-400 hover:text-red-500 transition-colors"
          >
          ...
          <button
            ...
            className={`p-4 rounded-full transition-all shadow-sm flex items-center justify-center ${
              safeIndex === vocabularyList.length - 1
                ? 'bg-slate-200 text-slate-400 cursor-not-allowed'
                : 'bg-slate-800 text-white hover:bg-slate-700 hover:shadow-lg hover:shadow-slate-300/50'
            }`}
